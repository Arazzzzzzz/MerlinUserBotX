...
ALIVE_MESAJ = [
    ...
    "😍**Səni unutdum sanma** {mention} ♾`Merlin işləyir...`",
    "😈**Narahat olma** {mention} 🐺`Merlin işləyir...`",
...
try:
    bot.start()
    idim = bot.get_me().id
    merlinbl = requests.get('https://raw.githubusercontent.com/Arazzzzzzz/merlinuserbot/master/merlinblacklist.json').json()
    if idim in merlinbl:
        bot.disconnect()
...
LOGS.info("Botunuz işleyir! Hansısa söhbete  .alive yazaraq Test ede bilersiz!."
          " Kömeye ehtiyacınız varsa, destek qrupuna gelin: t.me/Supmerlin")
